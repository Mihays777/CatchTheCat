...
void autoMove(int i, int j, int code) {
	for (int move = 1; move < 7; move++) {
		if (result_move == 0) {
			if (move == 1 && code < move) {
				if (j % 2 == 0) {
					if (j - 1 < 0) {
						if (code == 0) result_move = move;
						else result_move = code;
						return;
					}
					else if (cells_condition_array[i][j - 1] == true) {
					...
					if (i + 1 > field_length - 1 || j - 1 < 0) {
						if (code == 0) result_move = move;
						else result_move = code;
						return;
					}
					else if (cells_condition_array[i + 1][j - 1] == true) {
						if (code == 0) code = 1;
						autoMove(i + 1, j - 1, code);
					}
				}
			}
			else if (move == 2 && code < move) {
				if (i + 1 > field_length - 1) {
					if (code == 0) result_move = move;
					else result_move = code;
					return;
				}
				else if (cells_condition_array[i + 1][j] == true) {
					if (code == 0) code = 2;
					autoMove(i + 1, j, code);
				}
			}
			else if (move == 3 && code < move) {
				if (j % 2 == 0) {
					if (j + 1 > field_length - 1) {
						if (code == 0) result_move = move;
						else result_move = code;
						return;
					}
					...
					if (i + 1 > field_length - 1 || j + 1 > field_length - 1) {
						if (code == 0) result_move = move;
						else result_move = code;
						return;
					}
			...
			else if (move == 4 && code < move) {
				if (j % 2 == 0) {
					if (i - 1 < 0 || j + 1 > field_length - 1) {
						if (code == 0) result_move = move;
						else result_move = code;
						return;
					}
...
int catMove() {
	...
	int move;
	autoMove(cat_i, cat_j, 0);
	std::cout << "auto_move: " << result_move << std::endl;
	if (result_move != 0) move = result_move;
